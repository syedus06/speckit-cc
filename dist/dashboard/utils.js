 ...
 */
export async function isSpecificPortAvailable(port) {
 ...
 */
export async function checkExistingDashboard(port) {
    try {
 ...
 */
export async function validateAndCheckPort(port, skipDashboardCheck = false) {
 ...
 */
export function logProjectDiscovered(projectId, projectPath) {
    const event = {
 ...
 */
export function logAgentDetected(projectId, agentName, commandCount) {
    const event = {
 ...
 * Log scan completion
 */
export function logScanCompleted(rootDirectory, projectCount, scanDuration, errors) {
    const event = {
 ...
 * Log scan failure
 */
export function logScanFailed(rootDirectory, error) {
    const event = {
        event: 'scan.failed',
        timestamp: new Date().toISOString(),
        metadata: {
 ...
 */
export function logSpecParsed(projectId, specCount, structureComplexity) {
    const event = {
        event: 'spec.parsed',
        projectId,
        timestamp: new Date().toISOString(),
 ...
 */
export function logConstitutionAccessed(projectId, hasConstitution) {
    const event = {
 ...
 */
export async function timeOperation(operation, fn, metadata) {
 ...
 */
export function timeSyncOperation(operation, fn, metadata) {
    const start = performance.now();
    try {
        const result = fn();
        const duration = performance.now() - start;
