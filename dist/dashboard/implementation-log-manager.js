...
export class ImplementationLogManager {
    ...
    parseFileName(fileName) {
        ...
        if (parts.length < 3 || !parts[0].startsWith('task-'))
            return null;
    ...
    parseMarkdownContent(content) {
        try {
            ...
            const mapPropertyName = (normalizedKey) => {
                const mapping = {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                // Parse artifact subsections (### headers)
                    ...
                    if (Object.keys(currentItem).length > 0 && currentArtifactType) {
                        ...
                        artifacts[currentArtifactType].push(currentItem);
                        currentItem = {};
                    }
    ...
    async searchLogs(query) {
        ...
        return log.entries.filter(e => {
            ...
            return keywords.every(keyword => {
                ...
                if (e.summary.toLowerCase().includes(keyword) ||
                ...
                if (e.artifacts) {
                    ...
                    if (e.artifacts.apiEndpoints?.some(api => api.method?.toLowerCase().includes(keyword) ||
                    ...
                    // Search functions
                    if (e.artifacts.functions?.some(func => func.name?.toLowerCase().includes(keyword) ||
                    ...
                    // Search classes
                    if (e.artifacts.classes?.some(cls => cls.name?.toLowerCase().includes(keyword) ||
                    ...
                    // Search integrations
                    if (e.artifacts.integrations?.some(intg => intg.description?.toLowerCase().includes(keyword) ||
     ...
     */
    async getLogsByArtifactType(artifactType) {
        const log = await this.loadLog();
        return log.entries.filter(entry => entry.artifacts &&
            entry.artifacts[artifactType] &&
            entry.artifacts[artifactType].length > 0);
    }
    /**
     * Search for specific artifacts across all logs
     * Returns logs that contain artifacts matching the search term
     */
    async findArtifact(artifactType, searchTerm) {
        const log = await this.loadLog();
        const results = [];
        log.entries.forEach(entry => {
            if (entry.artifacts && entry.artifacts[artifactType]) {
                const artifacts = entry.artifacts[artifactType];
...
//# sourceMappingURL=implementation-log-manager.js.map