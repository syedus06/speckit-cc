...
export const getSpeckitAgentsTool = {
...
export const getSpeckitConstitutionTool = {
...
export const getSpeckitTemplatesTool = {
...
export const getSpeckitScriptsTool = {
...
export const getSpeckitProjectsTool = {
...
export const scanSpeckitRootTool = {
...
export async function getSpeckitAgentsHandler(args, context) {
        ...
        return {
            ...
            data: {
                ...
                agents: agents.map(agent => ({
                    ...
                    commands: agent.commands.map((cmd) => ({
...
export async function getSpeckitConstitutionHandler(args, context) {
    const { projectId } = args;
...
export async function getSpeckitTemplatesHandler(args, context) {
    const { projectId } = args;
    if (!projectId || typeof projectId !== 'string') {
        return {
            success: false,
...
export async function getSpeckitScriptsHandler(args, context) {
    const { projectId } = args;
    if (!projectId || typeof projectId !== 'string') {
        return {
            success: false,
...
export async function getSpeckitProjectsHandler(args, context) {
    ...
    try {
        ...
        if (includeDetails) {
            ...
            projectData = await Promise.all(specKitProjects.map(async (project) => {
                try {
                    const parser = project.parser;
                    const agents = await parser.discoverAIAgents();
            ...
            // Return summary only
            projectData = specKitProjects.map((project) => ({
                projectId: project.projectId,
                projectName: project.projectName,
                projectPath: project.projectPath,
                projectType: project.projectType,
                agentCount: project.agentCount || 0,
                specCount: project.specCount || 0,
                hasConstitution: project.hasConstitution || false
            }));
        }
        return {
            success: true,
...
export async function scanSpeckitRootHandler(args, context) {
...
//# sourceMappingURL=speckit-tools.js.map