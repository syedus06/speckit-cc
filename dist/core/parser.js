...
export class SpecKitParser {
    ...
    async parseAgentFolder(folderName) {
    ...
    async detectAgentSubdirectory(agentPath) {
    ...
    async parseAgentCommands(subdirPath, agentName) {
        const commands = [];
        try {
    ...
    calculateStructureComplexity(specs) {
        let complexity = 0;
    ...
    inferTemplateType(templateName) {
        if (templateName.includes('spec'))
            return 'spec';
        if (templateName.includes('plan'))
            return 'plan';
        if (templateName.includes('task'))
            return 'tasks';
        if (templateName.includes('checklist'))
            return 'checklist';
        return 'other';
    }
    async parseSpecDirectory(dirName) {
    ...
    async detectSpecFiles(specDirPath) {
        try {
            const entries = await readdir(specDirPath, { withFileTypes: true });
            // Check for standard files
            const hasSpec = entries.some((e) => e.name === 'spec.md');
            const hasPlan = entries.some((e) => e.name === 'plan.md');
            const hasTasks = entries.some((e) => e.name === 'tasks.md');
            // Get subdirectories
